...
def _main_menu_kb():
    return {"inline_keyboard": [
        ...
        [{"text": "📈 گزارش فروش", "callback_data": "salesstats"}],
...
def _store_kb():
    ...
    for pid,p in PLANS.items():
        rows.append([{"text": f"🛒 {p['name']} — {p['stars']} ⭐", "callback_data": f"buy:{pid}"}])
    rows.append([{"text":"🧾 خریدهای من","callback_data":"myorders"}])
    return {"inline_keyboard":rows}

def _store_text():
    return ("🛍 <b>VodiWalker Store</b>\n\n"
            "پلن موردنظر را انتخاب کن. بعد از پرداخت، کانفیگ اختصاصی به‌صورت خودکار ساخته و تحویل می‌شود.\n\n"
            "⚡ Starter: 30 روز / 50GB / 30Mbps\n"
            "🚀 Pro: 90 روز / 200GB / 80Mbps\n"
            "👑 Ultra: 180 روز / 500GB / 150Mbps")
...
async def _handle_message(msg: dict):
    ...
    if text.startswith("/start"):
        ...
        if _is_admin(chat_id):
            ...
            await _send(chat_id, "👋 <b>VodiWalker Control Center</b>\nمدیریت کامل سرویس و فروشگاه:", _main_menu_kb())
        else:
    ...
    if text == "/admin":
        await _send(chat_id, "🛠 <b>VodiWalker Control Center</b>", _main_menu_kb())
        return
...
async def _handle_callback(cb: dict):
    ...
    if data == "menu":
        _pending.pop(chat_id, None)
        await _edit(chat_id, message_id, "منوی مدیریت VodiWalker:", _main_menu_kb())
        return
